...
BEGIN
    v_count := (SELECT Count(*)
                FROM (SELECT processes.Decision.decision_id, Count(*)
                      ...
                      HAVING Count(*) < 2 FOR UPDATE) AS Decision_option_count);
    IF v_count > 0 THEN
...
CREATE FUNCTION processes.f_remove_process_step_with_next_step() RETURNS trigger AS $$
BEGIN
...
CREATE FUNCTION processes.f_edit_decision_option() RETURNS trigger AS $$
BEGIN
    IF ((SELECT processes.Process.process_status_type_code
         FROM processes.Process
...
CREATE FUNCTION processes.f_remove_decision_option() RETURNS trigger AS $$
BEGIN
    IF ((SELECT processes.Process.process_status_type_code
         FROM processes.Process
                  INNER JOIN (processes.Option INNER JOIN (processes.Decision INNER JOIN processes.Step ON processes.Decision.decision_id = processes.Step.step_id) ON OLD.decision_id = processes.Decision.decision_id)
...
CREATE FUNCTION processes.f_add_option() RETURNS trigger AS $$
BEGIN
    IF ((SELECT processes.Process.process_status_type_code
         FROM processes.Process
