...
CREATE OR REPLACE FUNCTION f_register_process(p_name Process.name%TYPE, p_description Process.description%TYPE, p_owner Process.owner_id%TYPE, p_password Process.password%TYPE)
RETURNS VOID AS $$
INSERT INTO Process(name, description, owner_id, password)
SELECT p_name, p_description, p_owner, p_password FOR UPDATE;
$$ LANGUAGE sql SECURITY DEFINER SET search_path = public, pg_temp;

COMMENT ON FUNCTION f_register_process(p_name Process.name%TYPE, p_description Process.description%TYPE, p_owner Process.owner_id%TYPE, p_password Process.password%TYPE) IS 'This function is used to register a new process.';
...
RETURNS VOID AS $$
INSERT INTO Variant(decision_id, weight, guard)
